...
class GPUSim:
    ...
    def particle_sim(self):
        ...
        print(cp.asnumpy(scatter_alpha))
        print(cp.asnumpy(scatter_step))
        ...
        for alpha, step in zip(scatter_alpha, scatter_step):
            ...
            step_energy = np.array(self.alpha_path.get())[step]
            ...
            self._alpha_sim.append(np.abs(np.diff(np.array(self.alpha_path.get())[0:step])))
            # compute scattering
